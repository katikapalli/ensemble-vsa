...
def average_voting(predictions, selected_models):
    ensemble_predictions = np.zeros_like(predictions[selected_models[0]])
    for model_name in selected_models:
        ensemble_predictions += predictions[model_name]
    ensemble_predictions /= len(selected_models)
...
def evaluate_ensemble(y_true, y_pred, args):
    ...
    filename = f'outputs/average_voting_{"proposed" if args.use_proposed_dcnn else "baseline"}_{args.num_models}.png'
...
def main():
    ...
    y_pred = average_voting(predictions, top_models)
