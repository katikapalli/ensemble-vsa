...
def weighted_voting(predictions, selected_models, model_weights):
    ensemble_predictions = np.zeros_like(predictions[selected_models[0]])

    for model_name in selected_models:
        ensemble_predictions += model_weights[model_name] * predictions[model_name]
...
def evaluate_ensemble(y_true, y_pred, args):
    ...
    filename = f'outputs/weighted_voting_{"proposed" if args.use_proposed_dcnn else "baseline"}_{args.num_models}.png'
...
def main():
    ...
    parser.add_argument('--use_proposed_dcnn', type=bool, default=False, help='Include ProposedDCNN model')
    parser.add_argument('--num_models', type=int, default=2, help='Number of top models to use')
    parser.add_argument('--model_performance', type=str, default='model_performance.json', help='Path to JSON with model accuracies')
    parser.add_argument('--step', type=float, default=0.1, help='Step size for weight adjustments')
    ...
    model_weights = {model_name: model_performance[model_name]['train_accuracy'] for model_name in models}
    print(model_weights)
    ...
    valid_dataset = AffectNetDataset(root_dir=config['dataset']['dataset_root'], transform=transform, config=config, split='valid')
    valid_dataloader = DataLoader(valid_dataset, batch_size=config['training']['batch_size'], shuffle=False, num_workers=4)


    models = load_models(model_dir, top_models)
    predictions, y_true = generate_predictions(models, valid_dataloader, device)

    y_pred = weighted_voting(predictions, top_models, model_weights)
